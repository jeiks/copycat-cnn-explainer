<script>
  ...
  import { array1d, compute_input_multiplies_with_weight,
          generateOutputMappings, copycatgridData
  ...
  // Fix the total copycatgrid size to change hyperparameters without changing the copycatgrid
  // size.  This must be two pixels smaller than the HyperparameterDataview svg
  // size, so that the stroke on the right side of the copycatgrid does not get cut off.
  const copycatgridSize = 198;
  startConvolution(stride);
  let testImage = copycatgridData(image, copycatgridSize / image.length);
  let testOutput = copycatgridData(output, copycatgridSize / output.length);
  let testKernel = copycatgridData(kernel, copycatgridSize / kernel.length);
  $ : {
    startConvolution(stride);
    testImage = copycatgridData(image, copycatgridSize / image.length);
    testOutput = copycatgridData(output, copycatgridSize / output.length);
    testKernel = copycatgridData(kernel, copycatgridSize / kernel.length);
  }
</script>
